...
fn supports_runtime_model_switch(channel_name: &str) -> bool {
    matches!(
        channel_name,
        "telegram" | "discord" | "matrix" | "slack" | "whatsapp" | "whatsapp-web" | "whatsapp_web"
    )
}
...
async fn process_channel_message(
    ...
    let mut typing_cancellation: Option<CancellationToken> = None;
        ...
        && let Some(channel) = target_channel.as_ref()
    {
        ...
        typing_cancellation = Some(token);
    }
    ...
    // Preserve a compact user turn before the LLM call so interrupted requests
    // keep context without persisting inline image payloads into future turns.
    append_sender_turn(
        ctx.as_ref(),
        &history_key,
        ChatMessage::user(&history_user_content),
    );
    ...
    // Strip [IMAGE:] markers from *older* history messages before context
    // compression. The current message keeps its full payload so vision still
    // works for the active turn; historical turns keep only compact annotations.
        ...
        LlmExecutionResult::Completed(Ok(Err(e))) => {
                ...
                || cancellation_token.is_cancelled()
            {
                ...
                let rolled_back = should_rollback_user_turn
                    && rollback_orphan_user_turn(ctx.as_ref(), &history_key, &history_user_content);
...
<tool_result name="http_request">{"status":200}</tool_result>"#;
    ...
    struct RecordingChannel {
        ...
        start_typing_notify: Option<Arc<tokio::sync::Notify>>,
    ...
    impl Channel for RecordingChannel {
        ...
        async fn start_typing(&self, _recipient: &str) -> anyhow::Result<()> {
            ...
            if let Some(notify) = &self.start_typing_notify {
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    #[async_trait::async_trait]
    impl Memory for SlowRecallMemory {
        fn name(&self) -> &str {
            "slow-recall"
        }
    ...
    #[tokio::test]
    async fn process_channel_message_starts_typing_before_slow_recall() {
        let start_notify = Arc::new(tokio::sync::Notify::new());
        let channel_impl = Arc::new(RecordingChannel {
            start_typing_notify: Some(Arc::clone(&start_notify)),
            ..Default::default()
        });
        let channel: Arc<dyn Channel> = channel_impl.clone();
        ...
        config.agent.precheck.enabled = false;
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            default_provider: Arc::new("test-provider".to_string()),
            memory: Arc::new(SlowRecallMemory {
        ...
        let process_task = tokio::spawn(process_channel_message(
            runtime_ctx,
            zeroclaw_api::channel::ChannelMessage {
                id: "typing-before-memory-msg".to_string(),
                sender: "alice".to_string(),
                reply_target: "chat-typing".to_string(),
                content: "hello".to_string(),
                channel: "test-channel".to_string(),
        ...
        let typing_started =
            tokio::time::timeout(Duration::from_millis(100), start_notify.notified()).await;
        process_task.await.unwrap();

        assert!(
            typing_started.is_ok(),
            "typing should start before slow memory recall finishes"
        );
    }
